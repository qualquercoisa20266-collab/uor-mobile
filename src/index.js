...
export class UORRoom {
 ...
 async fetch(req){const url=new URL(req.url),path=url.pathname; if(path.endsWith('/internal/init')){const b=await body(req);this.room={...b,hostConnId:b.hostConnId||b.players?.[0]?.id||null};await this.persist();return json({ok:true});}
